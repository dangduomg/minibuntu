...
systemd-nspawn -D root --machine=minibuntu-ice --as-pid2 bash -c "
...
systemd-nspawn -b -D root --machine=minibuntu-ice --bind=/etc/resolv.conf:/etc/resolv.conf > /dev/null &

until machinectl show minibuntu-ice | grep -q "State=running"; do
    sleep 1
done
machinectl shell minibuntu-ice /bin/bash -c "
    # external repo
...
"
machinectl poweroff minibuntu-ice
while machinectl list | grep -q minibuntu-ice; do
    sleep 1
done
