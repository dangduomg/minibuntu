...
systemd-nspawn -D root --machine=minibuntu-openbox-experiment --as-pid2 bash -c "
    ...
    # ui
    apt-get install --no-install-recommends -y \
        xorg openbox xfdesktop4 xfce4-panel xfce4-notifyd tumbler
    ...
    # theme
    apt-get install -y \
        lxappearance obconf fonts-ubuntu lubuntu-artwork \
        materia-gtk-theme papirus-icon-theme bibata-cursor-theme
...
systemd-nspawn -b -D root --machine=minibuntu-openbox-experiment > /dev/null &

until machinectl show minibuntu-openbox-experiment | grep -q "State=running"; do
    sleep 1
done
machinectl shell minibuntu-openbox-experiment /bin/bash -c "
    # external repo
...
"
machinectl poweroff minibuntu-openbox-experiment
while machinectl list | grep -q minibuntu-openbox-experiment; do
    sleep 1
done
